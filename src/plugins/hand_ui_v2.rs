use bevy::prelude::*;
use crate::components::combat::{Player, Enemy, HandArea, CombatUiRoot};
...
pub fn update_hand_ui_v2(
    ...
    if let Ok(player) = player_query.get_single() {
        if let Ok(hand) = hand_query.get_single() {
            ...
            for (hc, mut text) in text_queries.p3().iter_mut() {
                if let Some(card) = hand.cards.iter().find(|c| c.id == hc.card_id) {
    ...
    // 3. 仅在手牌容器变化时才重建实体
    if !hand_changed_query.is_empty() {
        if let Ok(hand) = hand_query.get_single() {
                ...
                for (i, card) in hand.cards.iter().enumerate() {
                    ...
                    commands.entity(hand_area_entity).with_children(|parent| {
                        parent.spawn((
                            ...
                            card_ui.spawn((Text::new(display_desc), TextFont { font: chinese_font.clone(), font_size: 11.0, ..default() }, TextColor(desc_color), TextLayout::new_with_justify(JustifyText::Center), Node { max_width: Val::Px(90.0), ..default() }));
                        });
                    });
                }
            }
